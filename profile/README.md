<div align="center">

<img width="800" height="400" alt="ut-rsoc" src="https://github.com/user-attachments/assets/e6c04e65-6dfe-4cc7-93f3-a64cfb911cf3" />

# UT-RSOC Student Researcher Program

**The University of Texas at Austin's Regional Security Operations Center**

*Empowering the Next Generation of Cybersecurity Professionals*

---

[![Discord](https://img.shields.io/badge/Discord-Join%20Us-7289da?logo=discord&logoColor=white)](https://discord.gg/E6FnWqCW)
[![GitHub](https://img.shields.io/badge/GitHub-UT--RSOC-181717?logo=github)](https://github.com/UT-RSOC)

</div>

---

## 🎓 About the Program

Welcome to the **UT-RSOC Student Researcher Program**! We are a dynamic cybersecurity student organization at The University of Texas at Austin, dedicated to providing hands-on experience in security operations, threat analysis, and incident response.

Our program offers students the unique opportunity to:

- 🔐 **Gain Real-World Experience** - Work on actual security operations and research projects
- 🚀 **Develop Professional Skills** - Learn industry-standard tools and methodologies
- 🤝 **Network with Professionals** - Connect with cybersecurity experts and peers
- 📚 **Access Exclusive Resources** - Get hands-on training and mentorship
- 🏆 **Build Your Resume** - Earn certifications and demonstrate practical skills

Whether you're just starting your cybersecurity journey or looking to deepen your expertise, the UT-RSOC Student Researcher Program provides a supportive environment to learn, grow, and make an impact in the field of cybersecurity.

<div align = "center">
<img width="308" height="227" alt="3" src="https://github.com/user-attachments/assets/a289cdd4-7b36-43c1-aef3-f1f18bb9a4b1" /><img width="308" height="227" alt="4" src="https://github.com/user-attachments/assets/c82c38a4-5b1a-46a3-84f9-6b26be269821" /><img width="308" height="227" alt="5" src="https://github.com/user-attachments/assets/3cf2a9f7-25c7-4a6a-8fed-051e1519baed" />
</div>

---

## 🚀 Getting Started

### Join Our Community

Ready to become part of the UT-RSOC team? Follow these steps:

1. **Join Our Discord Server** 🎮
   
   Click here to join: **[https://discord.gg/E6FnWqCW](https://discord.gg/E6FnWqCW)**
   
   Our Discord server is the hub of our community where you can:
   - Connect with fellow student researchers
   - Get real-time support and guidance
   - Stay updated on program activities and opportunities
   - Participate in discussions and knowledge sharing

2. **Request Repository Access** 📂
   
   Once you've joined Discord:
   - Introduce yourself in the appropriate channel
   - Request to be added as a repository member
   - Our Repo Manager will grant you access to view our repositories
   
   *Note: Repository access is required to participate in projects and access program resources.*

3. **Complete Onboarding** ✅
   
   After gaining access, you'll receive onboarding materials to help you:
   - Understand our tools and workflows
   - Learn about ongoing projects
   - Get matched with a mentor
   - Start contributing to the team

<!-- Placeholder for Footer/Banner Image -->
![Program Pathway](placeholder-footer.png)

---

## 👥 Program Coordinators

Our dedicated team is here to support your growth and success in the program:

### Leadership Team

**Jeremy Carter**  
*RSOC Manager*  
📧 [jeremy@utexas.edu](mailto:jeremy@utexas.edu)

**LaAudra Rasco**  
*RSOC Training Coordinator*  
📧 [laaudra@utexas.edu](mailto:laaudra@utexas.edu)

**Patrick Harris**  
*RSOC Senior Analyst*  
📧 [patrick@utexas.edu](mailto:patrick@utexas.edu)

### Student Leadership

**Robert Childers**  
*Discord Manager and Certified Student Analyst*  
📧 [rob01@utexas.edu](mailto:rob01@utexas.edu)

**Krish Prabhu**  
*Repo Manager and Student Analyst*  
📧 [prabhukrish710@utexas.edu](mailto:prabhukrish710@utexas.edu)

Feel free to reach out to any team member with questions or for guidance!

---

## 🎯 What We Do

### Security Operations
- Monitor and analyze security events
- Perform threat hunting and incident response
- Conduct vulnerability assessments

### Research & Development
- Explore emerging security threats
- Develop security tools and scripts
- Contribute to open-source security projects

### Training & Certification
- Participate in workshops and training sessions
- Work toward industry certifications
- Attend cybersecurity conferences and competitions

<img width="1382" height="993" alt="image" src="https://github.com/user-attachments/assets/4c67ade0-db21-4683-81e9-838e184fc8bd" />

---

## 💼 Opportunities

### For Students
- **Hands-on Experience**: Work on real security projects
- **Mentorship**: Learn from experienced professionals
- **Career Development**: Build skills that employers value
- **Networking**: Connect with the cybersecurity community
- **Recognition**: Earn certifications and recommendations

### Skills You'll Develop
- Security Information and Event Management (SIEM)
- Network Security Monitoring
- Threat Intelligence Analysis
- Incident Response and Forensics
- Security Automation and Scripting
- Communication and Teamwork

---

## 📚 Resources

Once you're a member, you'll have access to:

- 📖 Training materials and documentation
- 🛠️ Security tools and lab environments
- 📊 Project repositories and codebases
- 🎓 Certification study guides
- 🔗 Industry connections and job opportunities

---

## 🤝 Contributing

We welcome contributions from all skill levels! Whether you're:
- 🌱 A beginner eager to learn
- 💡 An intermediate student looking to apply your skills
- 🚀 An advanced practitioner ready to lead

There's a place for you in our program!

---

## 📞 Contact

Have questions? Need assistance? Reach out to us:

- **Discord**: [https://discord.gg/E6FnWqCW](https://discord.gg/E6FnWqCW)
- **Email**: Contact any of our program coordinators listed above
- **GitHub**: [UT-RSOC Organization](https://github.com/UT-RSOC)

---

## 🎉 Join Us Today!

Don't miss this opportunity to be part of one of UT Austin's premier cybersecurity programs. Join our Discord, get repository access, and start your journey in cybersecurity!

**[🎮 Join Discord Now](https://discord.gg/E6FnWqCW)**

---

<div align="center">

<img width="1440" height="500" alt="image" src="https://github.com/user-attachments/assets/b67c2690-a469-4597-be1d-ce68270fe2a7" />

**© 2024 UT-RSOC Student Researcher Program**  
*The University of Texas at Austin*

*Hook 'em Horns! 🤘*

</div>
